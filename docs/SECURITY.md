...
- Displayed token counts come from allowlisted fields in authenticated CLIProxyAPI usage records retained by FableMaxxing. The spend ledger groups those records into Fable/Anthropic and OpenAI/Codex lanes independently of route attestation. Claude Code transcript files are not read; assistant content, user content, prompts, tool calls, and transcript identifiers never enter the telemetry path.
- Raw proxy usage records are reduced to token counts, latency, the configured harness alias, provider/model spend classification, route evidence, and success state before mode-`0600` persistence. The browser receives sanitized model names and route attestation, but never request IDs, endpoints, source identities, or credentials.
